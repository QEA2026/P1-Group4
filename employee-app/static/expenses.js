...
class ExpenseManager {
  ...
  async checkAuthStatus() {
    try {
      ...
      if (
        data.authenticated &&
        data.user.role.toLowerCase() === "employee"
      ) {
  ...
  setupEventListeners() {
    ...
    document
      .getElementById("refresh-expenses")
  ...
  async submitExpense() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showMessage(
          "submit-message",
          data.error || "Failed to submit expense",
          "error",
        );
      }
    } catch (error) {
  ...
  async loadExpenses() {
    ...
    const url = statusFilter
      ? `/api/expenses?status=${statusFilter}`
      : "/api/expenses";
  ...
  displayExpenses(expenses) {
    ...
    html +=
      '<tr style="background-color: #f0f0f0;"><th>Date</th><th>Amount</th><th>Description</th><th>Status</th><th>Comment</th><th>Actions</th></tr>';
  ...
  editExpense(expenseId) {
    ...
    if (!expense) {
      alert("Expense details are no longer available. Please refresh.");
      return;
    }
